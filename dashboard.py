...
async function load(){
  ...
  if(d.tick){const T=d.tick,TR=T.rules||{};
    ...
    $('tktiles').innerHTML=[
      ...
      tile('Lanes',Object.keys(lanes).length?Object.keys(lanes).map(function(k){return k+' '+M(lanes[k].pnl)}).join(' &middot; '):'&ndash;','<b>fav</b> = buy the FAVOURITE at '+((TR.fav_band||[80,95])[0])+'-'+((TR.fav_band||[80,95])[1])+'&cent; in the last '+(TR.fav_at_s||90)+'s &mdash; measured +9.2&cent;/trade over 85 real windows &middot; endgame = certainty the clock already delivered &middot; tail = the longshot-bias harvest &middot; exit = sold back before settlement'),
